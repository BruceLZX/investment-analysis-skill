...
- Read `references/company_deep_dive_playbook.md` for **every** company. This is the mandatory forensic depth standard: legal entity map with exact ownership %, complete timeline with dates, equity cap table with share classes and voting rights, per-person founder/executive CVs with education and career chronology, patent portfolio table with numbers/dates/jurisdictions, per-product forensic detail, domestic vs. international market bifurcation, and complete competitor landscape.
- After drafting the report, read `references/investor_reviewer_lens.md` and run a full 5-round investor committee review. All Critical items must be resolved. The reviewer produces `_work/investor_review.md`.
- Keep the research plan, source register, evidence matrix, thesis table, red-team review, investor review, and all scratch work under `_work/`.
